...
def memetic_algorithm(matrix, generations, population_size):
	...
	local_search_iterations = 400 # Iterations of each local search performed
	...
	for generation in xrange(generations): # Main loop
		...
		if not generation % 10: # Optimize 10% of the individuals every 10 generations with local search

			selected = random.sample(range(len(old_population)), int(0.1*len(old_population)))

			for individual in selected:

				old_population[individual][0] = TSPLocalSearch.local_search(matrix, old_population[individual][1], local_search_iterations)
...
if __name__ == "__main__":
	...
	before = time.clock()
	path, cost = memetic_algorithm(matrix, generations, population_size)
	after = time.clock()
